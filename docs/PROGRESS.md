...
> 👉 SIGUIENTE: **Step 15 del chatbot** — `/api/inngest` SANO en prod (500→401) y **Inngest Cloud sincronizado** (4 funciones, URL estable). Falta: **ingesta end-to-end en prod** (subir doc en `/knowledge` → `ready`, requiere login Clerk) + dominio `panel.<dominio>` (Cloudflare CNAME→Vercel) + instancia de **producción de Clerk** + claves Meta reales para el Step 16.
>
