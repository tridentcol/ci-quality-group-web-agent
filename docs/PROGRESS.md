...
> 👉 SIGUIENTE: **Step 15 del chatbot** — casi cerrado. ✅ `/api/inngest` sano, ✅ Inngest Cloud sincronizado, ✅ **ingesta end-to-end en prod** (PDF → `Listo`), ✅ **dominio `bot.ci-quality-group.com`** (SSL emitido). **Falta solo:** instancia de **producción de Clerk** (hoy es dev → en `bot.ci-quality-group.com` el login da `dev-browser-missing`) + claves Meta reales para el Step 16. (El panel del chatbot vive en `bot.` porque `panel.`/`admin.` son de otro panel.)
>
...
- [x] Desplegado en `bot.ci-quality-group.com` y estable (dominio + SSL; falta login con Clerk producción)
