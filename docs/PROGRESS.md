...
> 👉 SIGUIENTE: **Step 12 del chatbot** — Panel: Dashboard + Settings. `(panel)/dashboard` con KPIs (conversaciones, leads, handoffs, huecos) y `(panel)/settings` + `api/panel/config` (GET/PATCH `bot_config`: nombre/tono, bienvenida, fuera de horario, horarios, toggles de canal, WhatsApp admin, retención, descuento máximo).
>
