...
> 👉 SIGUIENTE: **Step 15 del chatbot** — `/api/inngest` ya está SANO en prod (500→401, ver abajo). Falta: **sync de la app en Inngest Cloud** apuntando a `https://ci-quality-group-chatbot.vercel.app/api/inngest` (PUT firmado desde el dashboard de Inngest) + dominio `panel.<dominio>` (Cloudflare CNAME→Vercel) + instancia de **producción de Clerk** + claves Meta reales para el Step 16.
>
...
- [ ] **Step 15** — Deploy (Vercel + Blob + Inngest; subdominio `panel.<dominio>`) — en curso (2026-06-04): `BLOB_READ_WRITE_TOKEN` y `INNGEST_EVENT_KEY`/`INNGEST_SIGNING_KEY` ya en Vercel **Production**. `/api/inngest` **sano** (500→401, parsers nativos a `import()` diferido en `parse.ts`, commit `cd654e5`): el SDK responde 401 al GET sin firma (`x-inngest-sdk-handled: true`) y registra las 4 funciones. Recordar: cada deployment congela las env vars de su creación → tras cambiar llaves, **redeploy**. **Pendiente:** (1) **sync** de la app en Inngest Cloud → `https://ci-quality-group-chatbot.vercel.app/api/inngest`; (2) dominio `panel.<dominio>` (Cloudflare CNAME→Vercel); (3) instancia de **producción de Clerk**; (4) ingesta end-to-end en prod (subir doc → `ready`).
