...
- [ ] **Layout 100% responsive + navegación mobile excelente**: el panel `(panel)/*` debe verse y navegarse impecable en móvil/tablet/desktop. Sidebar → menú colapsable/drawer en móvil, tablas (`leads`/`conversations`/`pricing`) con scroll/cards en pantallas chicas, formularios (`settings`/`pricing`) usables con el pulgar, hilos de conversación cómodos en móvil. Revisar TODAS las páginas del panel.
- [ ] **Cargar conocimiento real** en `/knowledge` (servicios, ubicaciones, horarios, FAQ, proceso de chatarrización/disposición) — el RAG está vacío de contenido real.
...
- [ ] **Probar y afinar el bot con el harness** `pnpm --filter chatbot chat` (REPL que ejercita memoria+RAG+router+tools sin Meta; `/rag <texto>` inspecciona cobertura). Verificar que no alucina, respeta precios/descuentos y deriva bien.
