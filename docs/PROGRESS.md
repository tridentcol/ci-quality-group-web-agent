...
> 👉 SIGUIENTE: **Step 11 del chatbot** — Panel: Leads + Conversaciones + Huecos. `api/panel/leads` (GET/PATCH: aprobar descuento, cambiar estado) + `api/panel/conversations` (lista por canal, hilo, toggle tomar/liberar = status human_controlled↔bot_active) + `api/panel/gaps` (resolver inline → crea fuente `faq` y la ingiere). Páginas `(panel)/{leads,conversations,gaps}`.
>
