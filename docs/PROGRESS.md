...
- [ ] **Step 15** — Deploy (Vercel + Blob + Inngest; subdominio `panel.<dominio>`) — en curso (2026-06-03): `BLOB_READ_WRITE_TOKEN` y `INNGEST_EVENT_KEY`/`INNGEST_SIGNING_KEY` ya en Vercel **Production**. Recordar: cada deployment congela las env vars de su creación → tras añadir/cambiar llaves hay que **redeploy** para que tomen efecto. Verificación: `GET /api/inngest` debe pasar de 500 → 200. Pendiente: sync de la app en Inngest Cloud + dominio `panel.<dominio>` (Cloudflare CNAME→Vercel) + instancia de producción de Clerk.
