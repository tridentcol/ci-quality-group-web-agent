...
> 👉 SIGUIENTE: **Step 10 del chatbot** — Integración Meta (webhook unificado): `lib/meta/{verify,normalize,send,notify}.ts` + `app/api/webhooks/meta/route.ts` (GET verify + POST con idempotencia vía `webhook_events`, detección de echo/relevo, pipeline `loadMemory→generateReply→appendMessage→send`, respuesta <20s con `waitUntil`). Completar el envío real en `lib/meta/notify.ts:notifyAdmin` (hoy es stub).
>
