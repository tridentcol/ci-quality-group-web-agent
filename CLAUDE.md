...
- apps/website   → Astro, <dominio> (Vercel). Guía: docs/ci-quality-group-website-blueprint.md
...
5. Cada app despliega por separado (proyecto Vercel propio, Root Directory distinto). No acoples sus configs.
...
## Deploys (mismo repo, dos proyectos Vercel)
- Vercel (chatbot): Root Directory = `apps/chatbot`.
- Vercel (website): Root Directory = `apps/website`; build `pnpm install && pnpm --filter website build`, output `apps/website/dist`. DNS en Cloudflare (solo nameservers, no Pages).
